...
чет не отправило ничего
Time Capsule, [17.03.2025 21:21]
📦 Введите номер капсулы для действия:

Lunaris, [17.03.2025 21:21]
...
Time Capsule, [17.03.2025 21:22]
📅 Вы выбрали дату: 17.03.2025 21:23
Капсула будет отправлена в указанное время.

Lunaris, [17.03.2025 21:22]
21:23 17.03.2025

вот код для отложенной отправки из прошлых версий
...
async def send_capsule_task(capsule_id):
    ...
    for recipient in recipients:
        chat_id = get_chat_id(recipient['recipient_username'])
        if chat_id:
            await bot.send_message(chat_id=chat_id, text=i18n.t('capsule_received', sender=capsule['creator_id']))
...
# Функция для установки даты отправки капсулы
async def handle_select_send_date(update: Update, context: CallbackContext):
    try:
        if context.user_data.get('state') == "selecting_send_date":
            ...
            scheduler.add_job(send_capsule_task, 'date', run_date=send_date_utc, args=[capsule_id], id=f"capsule_{capsule_id}", timezone=pytz.utc)
...
# Функция для инициализации задач после запуска приложения
async def post_init(application):
    capsules = fetch_data("capsules")
    now = datetime.now(pytz.utc)
    for capsule in capsules:

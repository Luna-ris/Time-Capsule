...
def send_capsule_task(capsule_id: int):
    async def send_async():
        try:
            logger.info(f"[Celery] Начинаю отправку капсулы {capsule_id}")
            
            capsule = fetch_data("capsules", {"id": capsule_id})
            if not capsule:
                logger.error(f"[Celery] Капсула {capsule_id} не найдена")
                return
            
            logger.info(f"[Celery] Данные капсулы: {capsule}")
            
            content = json.loads(decrypt_data_aes(capsule[0]['content'], ENCRYPTION_KEY_BYTES))
            recipients = get_capsule_recipients(capsule_id)
            
            if not recipients:
                logger.error(f"[Celery] Нет получателей для капсулы {capsule_id}")
                return
            ...
            for recipient in recipients:
                ...
                if chat_id:
                    logger.info(f"[Celery] Отправляю капсулу {capsule_id} пользователю {recipient['recipient_username']}")
                    
                    await bot.bot.send_message(
                        chat_id=chat_id,
                        text=t('capsule_received', sender=sender_username, locale='en')  # или нужная локаль
                    )
                    ...
                    logger.info(f"[Celery] Капсула {capsule_id} успешно отправлена @{recipient['recipient_username']}")
                else:
                    logger.warning(f"[Celery] Получатель @{recipient['recipient_username']} не зарегистрирован")
            
            logger.info(f"[Celery] Капсула {capsule_id} успешно отправлена всем получателям")
            delete_capsule(capsule_id)
        except Exception as e:
            logger.error(f"[Celery] Ошибка в задаче отправки капсулы {capsule_id}: {e}")
